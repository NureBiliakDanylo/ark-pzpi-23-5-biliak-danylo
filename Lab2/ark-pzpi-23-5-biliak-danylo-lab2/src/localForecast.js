...
async function generateLocalForecast(sensorId, hoursAhead = 1) {
  const readings = await new Promise((resolve, reject) => {
    db.all(`SELECT * FROM sensor_readings WHERE sensor_id = ? ORDER BY created_at DESC LIMIT 10`, [sensorId], (err, rows) => {
      if (err) reject(err);
  ...
  const insertSql = `INSERT INTO local_forecasts (sensor_id, forecast_time, predicted_temp, predicted_humidity, predicted_pressure) VALUES (?, ?, ?, ?, ?)`;
  const forecastId = await new Promise((resolve, reject) => {
    db.run(insertSql, [
      sensorId,
  ...
  return {
    ...
    note: "This forecast is based on a linear regression of the last 10 sensor readings. Its accuracy decreases with the forecast horizon."
  };
}
